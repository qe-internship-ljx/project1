# experiment3 — Euro Stoxx 50 Cross-Market (Euro Stoxx 50 / VSTOXX Port)

Port of the `experiment2 - Predictive Return Regression and Timing Backtests`
timing pipeline to the **European market**: every signal is re-derived from European instruments and
...
by `experiment3 - Nasdaq 100 Cross-Market`.
...
```bash
python "experiment3 - Euro Stoxx 50 Cross-Market/euro_experiment.py"
```

Self-sufficient with respect to VRP (recomputes it), but requires the sibling
directories `experiment2 - Predictive Return Regression and Timing Backtests`,
`experiment1 - HAR VRP Computation` (code, not output) and `Bekaert_Hoerova_Replication` on
disk — imports are wired through
