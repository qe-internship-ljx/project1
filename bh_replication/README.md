# bh_replication — Bekaert & Hoerova (2014) HAR-RV-VIX Baseline
...
1. **RV proxy** — daily squared front-month ES futures returns `(r·100)²`
   instead of 5-minute intraday RV (see `intraday_experiment/` for the
   intraday version).
...
`har_model.py` are imported by `experiment1 - VRP Computation`,
`intraday_experiment`, and (indirectly) `experiment2 - Return Regression`.
...
```bash
python bh_replication/run_replication.py   # writes the two PNGs
python bh_replication/run_comparison.py    # prints the VIX-vs-VS comparison
```
