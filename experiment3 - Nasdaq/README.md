# experiment3 — Nasdaq (NASDAQ-100 Port)
...
| VRP (`VP`) | `experiment1 - VRP Computation/output/production_loop_expanding.csv` (expanding-window US VRP) via experiment2's `load_vrp_series_expanding()` |
| Term slope | `fh_replication.compute_vix_term_slope()` on VIX futures |
| VVIX MA5 | experiment2's `compute_vvix_ma5(load_vvix())` |

The regression/backtest engine is delegated entirely to
`cross_market.run_all` (which lives in `experiment3 - Euro/cross_market.py`
and drives experiment2's `regressions` / `base_strategies` /
...
python "experiment3 - Nasdaq/nasdaq_experiment.py"
```

Requires the sibling directories `experiment2 - Return Regression`,
`experiment3 - Euro` (for `cross_market.py`), `fh_replication` and
`bh_replication` on disk (wired via `sys.path` inserts). Reads
