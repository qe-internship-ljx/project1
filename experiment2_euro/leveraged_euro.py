...
leveraged_euro.py
=================
Unbound multi-level position sizing for Euro Stoxx 50 strategies.
Mirrors experiment2/leveraged_strategies.py for European markets.
...
Threshold types:
  ...
  asymmetric:        long:  excess = y_hat - mu       (enters only if >= 0.2%)
                     short: excess = -y_hat           (y_hat < 0, abs >= 0.2%)
  base_return_shift: excess = y_hat - mu              (symmetric around rolling mu)

Models:
  VV2TX MA5   univariate              -> output/expanding_window/VV2TX MA5/
  VRP         univariate              -> output/expanding_window/VRP/
  VRP + VV2TX MA5  bivariate          -> output/expanding_window/VRP + VV2TX MA5/
  VRP + Term Slope bivariate          -> output/expanding_window/VRP + Term Slope/

Outputs:
  VV2TX MA5/leveraged_symmetric_VV2TX_MA5.png
  VV2TX MA5/leveraged_asymmetric_VV2TX_MA5.png
  VRP/leveraged_symmetric_VRP.png
  VRP/leveraged_asymmetric_VRP.png
  VRP/leveraged_base_return_shift_VRP.png
  VRP + VV2TX MA5/leveraged_symmetric_VRP_+_VV2TX_MA5.png
  VRP + VV2TX MA5/leveraged_asymmetric_VRP_+_VV2TX_MA5.png
  VRP + VV2TX MA5/leveraged_base_return_shift_VRP_+_VV2TX_MA5.png
  VRP + Term Slope/leveraged_symmetric_VRP_+_Term_Slope.png
  VRP + Term Slope/leveraged_asymmetric_VRP_+_Term_Slope.png
  VRP + Term Slope/leveraged_base_return_shift_VRP_+_Term_Slope.png
  comparisons/leveraged_asymmetric_vv2tx_vs_vrp_vv2tx.png
"""
...
from euro_experiment import (
    load_stoxx_front_month, load_v2x_spot, load_vstoxx_futures, load_vv2tx,
    compute_euro_ivar, compute_euro_vrp,
    compute_vstoxx_term_slope, compute_vv2tx_ma5,
    build_euro_panel,
    _nw_se,
    simulate_strategy, compute_performance_stats, compute_buy_and_hold,
    OOS_START, MIN_WIN, T_THRESH, NW_LAGS, ROLL_VRP_WIN,
    _shade,
)
...
C_VV2TX = "#7f2704"   # VV2TX MA5 — dark orange/brown (Model_VV2X palette[0])
C_VRP   = "#08306b"   # VRP       — dark blue         (Base palette[0])
C_BIV   = "#3f007d"   # VRP+VV2TX — purple            (Model_C palette[0])
C_VRP2  = "#2171b5"   # second t-stat colour in bivariate VV2TX panel
C_TERM  = "#00441b"   # VRP+Term Slope — dark green   (Model_A palette[0])
...
# ═══════════════════════════════════════════════════════════════════════════
# Beta computation  (euro-specific cache keys, standard column names)
# ═══════════════════════════════════════════════════════════════════════════

def compute_betas_univ(panel, predictor, fwd_col, oos_gap, nw_lags):
    """Expanding-window OLS for a single predictor.  Cached."""
    tag   = f"euro_lev_betas_univ_{predictor}_{fwd_col}_oos{OOS_START}.parquet"
    cache = CACHE_DIR / tag
    if cache.exists():
        return pd.read_parquet(cache)

    print(f"    Computing univariate betas: {predictor} -> {fwd_col}...")
...
def compute_betas_biv(panel, pred1, pred2, fwd_col, oos_gap, nw_lags):
    """Expanding-window OLS for two predictors.  Cached."""
    tag   = f"euro_lev_betas_biv_{pred1}_{pred2}_{fwd_col}_oos{OOS_START}.parquet"
    cache = CACHE_DIR / tag
    if cache.exists():
        return pd.read_parquet(cache)

    print(f"    Computing bivariate betas: {pred1} + {pred2} -> {fwd_col}...")
...
def run_ew_unbound_sym(panel, predictor, fwd_col, oos_gap, nw_lags):
    ...
    tag   = (f"euro_pos_EW_unbound_sym_{predictor}_{fwd_col}"
...
def run_ew_unbound_asym(panel, predictor, fwd_col, oos_gap, nw_lags,
    ...
    tag   = (f"euro_pos_EW_unbound_asym_{predictor}_{fwd_col}"
...
def run_ew_unbound_rolmu(panel, predictor, fwd_col, oos_gap, nw_lags,
    ...
    tag   = (f"euro_pos_EW_rolmu_unbound_{predictor}_{fwd_col}"
             f"_t{int(T_THRESH*100)}_rw{rolling_window}_oos{OOS_START}.parquet")
    cache = CACHE_DIR / tag
    if cache.exists():
...
def run_ew_biv_unbound_sym(panel, pred1, pred2, fwd_col, oos_gap, nw_lags):
    tag   = (f"euro_pos_EW_unbound_sym_{pred1}_{pred2}_{fwd_col}"
             f"_t{int(T_THRESH*100)}_oos{OOS_START}.parquet")
    cache = CACHE_DIR / tag
    if cache.exists():
...
def run_ew_biv_unbound_asym(panel, pred1, pred2, fwd_col, oos_gap, nw_lags,
                             rolling_window=500):
    tag   = (f"euro_pos_EW_unbound_asym_{pred1}_{pred2}_{fwd_col}"
             f"_t{int(T_THRESH*100)}_rw{rolling_window}_oos{OOS_START}.parquet")
    cache = CACHE_DIR / tag
    if cache.exists():
...
def run_ew_biv_unbound_rolmu(panel, pred1, pred2, fwd_col, oos_gap, nw_lags,
                              rolling_window=500):
    tag   = (f"euro_pos_EW_rolmu_unbound_{pred1}_{pred2}_{fwd_col}"
             f"_t{int(T_THRESH*100)}_rw{rolling_window}_oos{OOS_START}.parquet")
    cache = CACHE_DIR / tag
    if cache.exists():
...
def _yhat_univ(panel, predictor, fwd_col, betas_df):
    sub = panel.dropna(subset=[predictor, fwd_col]).copy()
...
def _yhat_biv(panel, pred1, pred2, fwd_col, betas_df):
    sub = panel.dropna(subset=[pred1, pred2, fwd_col]).copy()
    idx = betas_df.index.intersection(sub.index)
    return (betas_df.loc[idx, "alpha"]
...
def _oos_cumret(sim, start=OOS_START):
    net = sim["net_pnl"]
...
def _draw_cumret(ax, sim, bah_sim, main_color, label):
    """Panel 1 — cumulative net return. Returns (_stat_start, pL, pS, avg_pos)."""
    oos_dt  = pd.Timestamp(OOS_START)
    pos_oos = sim["position"][sim.index >= OOS_START]
    active  = pos_oos[pos_oos != 0]
    _act    = active.index.min() if len(active) else None
    _rebase = None
    if _act is not None and _act > pd.Timestamp("2020-01-01"):
        idx_arr = bah_sim.index
        act_i   = idx_arr.searchsorted(_act)
        _rebase = idx_arr[min(act_i + 1, len(idx_arr) - 1)]
    _stat_start = _rebase if _rebase is not None else oos_dt
    _stat_lbl   = (f" stats from {_stat_start.strftime('%Y-%m-%d')}"
                   if _rebase is not None else "")

    _bah_st = compute_performance_stats(bah_sim[bah_sim.index >= _stat_start], "BaH")
    ax.plot(_oos_cumret(bah_sim).index, _oos_cumret(bah_sim).values,
    ...
    ax.plot(_oos_cumret(sim).index, _oos_cumret(sim).values,
...
def _draw_tstat_univ(ax, betas_df, main_color, pred_label, nw_lags):
    """Panel 2 — univariate t-stat and beta."""
    ...
    if "r2_insample" in betas_df.columns:
        ax2.plot(betas_df["r2_insample"].index, betas_df["r2_insample"].values,
                 color="forestgreen", lw=0.9, ls=":", alpha=0.75, label="In-sample R²")
...
def plot_unbound_bivariate(pred1_label, pred2_label, horizon_label, threshold_type,
    ...
    oos_dt = pd.Timestamp(OOS_START)
    e_dt   = betas_df.index[-1]
    if c1 is None: c1 = C_VRP
    if c2 is None: c2 = C_VRP2
    ...
    _, pL, pS, avg_pos = _draw_cumret(ax_ret, sim, bah_sim, main_color, "Unbound +/-1..4")
    _draw_tstat_biv(ax_t, betas_df, c1, c2, pred1_label, pred2_label, nw_lags)
    _draw_position(ax_p, sim, main_color, pL, pS, avg_pos)
    _draw_predicted(ax_yh, y_hat_ser, mu_ser, main_color, threshold_type)
    _finalize(fig, axes, out_path)


def plot_unbound_comparison(sim_vv2tx, sim_biv, sim_term, bah_sim, out_path,
                            sim_vrp=None):
    """One-panel comparison: unbound-asymmetric VV2TX MA5 / VRP+VV2TX MA5 / VRP+Term Slope."""
    oos_dt         = pd.Timestamp(OOS_START)
    ...
    candidates = [d for d in [_first_active(s)
                               for s in (sim_vv2tx, sim_biv, sim_term)]
                  if d is not None]
    ...
    bah_st  = _stats(bah_sim,   "BaH")
    vv2_st  = _stats(sim_vv2tx, "VV2TX_asym")
    biv_st  = _stats(sim_biv,   "BIV_asym")
    term_st = _stats(sim_term,  "TERM_asym")

    pL_v, pS_v, avg_v = _pos_pct(sim_vv2tx)
    pL_b, pS_b, avg_b = _pos_pct(sim_biv)
    pL_t, pS_t, avg_t = _pos_pct(sim_term)

    all_sims = [sim_vv2tx, sim_biv, sim_term]
    ...
    fig.suptitle(
        "Unbound Asymmetric: VV2TX MA5 vs VRP + VV2TX MA5 vs VRP + Term Slope  "
        "·  Expanding Window  ·  Euro Stoxx 50 (FESX)\n"
        f"Rebased to 1.0 at {start_str}  ·  |t| > {T_THRESH:.2f} gate  ·  "
        f"Levels ±1..4 at |excess| ≥ 0.2/0.5/0.75/1.0%  ·  0.05% slippage",
        fontsize=10,
    )

    ax.plot(_cum(bah_sim).index, _cum(bah_sim).values,
    ...
    ax.plot(_cum(sim_vv2tx).index, _cum(sim_vv2tx).values,
            color=C_VV2TX, lw=2.2, ls="-", alpha=0.92,
            label=(f"VV2TX MA5 (Unbound Asym)  "
                   f"[SR={vv2_st['sharpe']:+.2f}  "
                   f"ret={vv2_st['ann_ret']*100:+.1f}%  "
                   f"DD={vv2_st['max_dd']*100:.1f}%  "
                   f"L{pL_v:.0f}%/S{pS_v:.0f}%  AvgPos={avg_v:+.2f}]"))

    ax.plot(_cum(sim_biv).index, _cum(sim_biv).values,
            color=C_BIV, lw=2.2, ls="--", alpha=0.92,
            label=(f"VRP + VV2TX MA5 (Unbound Asym)  "
                   f"[SR={biv_st['sharpe']:+.2f}  "
                   f"ret={biv_st['ann_ret']*100:+.1f}%  "
                   f"DD={biv_st['max_dd']*100:.1f}%  "
                   f"L{pL_b:.0f}%/S{pS_b:.0f}%  AvgPos={avg_b:+.2f}]"))

    ax.plot(_cum(sim_term).index, _cum(sim_term).values,
    ...
    for a_dt, b_dt in [("2020-02-01", "2020-06-01"), ("2022-01-01", "2022-12-31")]:
        ax.axvspan(pd.Timestamp(a_dt), pd.Timestamp(b_dt),
...
def main():
    print("=" * 72)
    print("  Euro Leveraged Strategies  (Euro Stoxx 50 / FESX)")
    print("  VV2TX MA5 / VRP / VRP+VV2TX MA5 / VRP+Term Slope")
    print("  x  sym / asym / rolmu")
    print("=" * 72)
    ...
    print("\n[2] Computing / loading betas (cached)...")
    betas_vv2tx = compute_betas_univ(panel, "vv2tx_ma5",  FWD, OOS_GAP, NW_LAGS)
    betas_vrp   = compute_betas_univ(panel, "VP",         FWD, OOS_GAP, NW_LAGS)
    betas_biv   = compute_betas_biv(panel,  "VP", "vv2tx_ma5",  FWD, OOS_GAP, NW_LAGS)
    betas_term  = compute_betas_biv(panel,  "VP", "term_slope", FWD, OOS_GAP, NW_LAGS)
    print("    Done.")

    yh_vv2tx = _yhat_univ(panel, "vv2tx_ma5",  FWD, betas_vv2tx)
    yh_vrp   = _yhat_univ(panel, "VP",          FWD, betas_vrp)
    yh_biv   = _yhat_biv(panel,  "VP", "vv2tx_ma5",  FWD, betas_biv)
    yh_term  = _yhat_biv(panel,  "VP", "term_slope", FWD, betas_term)
    mu_20d   = _rolling_mu(panel, FWD, OOS_GAP)

    out_vv2tx = OUTPUT / "expanding_window" / "VV2TX MA5"
    out_vrp   = OUTPUT / "expanding_window" / "VRP"
    out_biv   = OUTPUT / "expanding_window" / "VRP + VV2TX MA5"
    out_term  = OUTPUT / "expanding_window" / "VRP + Term Slope"
    out_cmp   = OUTPUT / "expanding_window" / "comparisons"
    for d in (out_vv2tx, out_vrp, out_biv, out_term, out_cmp):
        d.mkdir(parents=True, exist_ok=True)

    # ── A: VV2TX MA5 symmetric ───────────────────────────────────────────────
    print("\n[A] VV2TX MA5 unbound symmetric...")
    pos = run_ew_unbound_sym(panel, "vv2tx_ma5", FWD, OOS_GAP, NW_LAGS)
    sim = _report(pos, daily_ret, "ub_sym_VV2TX")
    plot_unbound_univariate(
        "VV2TX MA5", "20-day", "sym", C_VV2TX,
        sim, betas_vv2tx, bah_sim, yh_vv2tx, mu_20d,
        out_vv2tx / "leveraged_symmetric_VV2TX_MA5.png",
    )

    # ── B: VV2TX MA5 asymmetric ──────────────────────────────────────────────
    print("\n[B] VV2TX MA5 unbound asymmetric...")
    pos = run_ew_unbound_asym(panel, "vv2tx_ma5", FWD, OOS_GAP, NW_LAGS)
    sim_vv2tx_asym = _report(pos, daily_ret, "ub_asym_VV2TX")
    plot_unbound_univariate(
        "VV2TX MA5", "20-day", "asym", C_VV2TX,
        sim_vv2tx_asym, betas_vv2tx, bah_sim, yh_vv2tx, mu_20d,
        out_vv2tx / "leveraged_asymmetric_VV2TX_MA5.png",
    )
    ...
    # ── F: VRP+VV2TX MA5 bivariate symmetric ─────────────────────────────────
    print("\n[F] VRP+VV2TX MA5 unbound symmetric...")
    pos = run_ew_biv_unbound_sym(panel, "VP", "vv2tx_ma5", FWD, OOS_GAP, NW_LAGS)
    sim = _report(pos, daily_ret, "ub_sym_biv")
    plot_unbound_bivariate(
        "VRP", "VV2TX MA5", "20-day", "sym", C_BIV,
        sim, betas_biv, bah_sim, yh_biv, mu_20d,
        out_biv / "leveraged_symmetric_VRP_+_VV2TX_MA5.png",
        c1=C_VRP, c2=C_VV2TX,
    )

    # ── G: VRP+VV2TX MA5 bivariate asymmetric ────────────────────────────────
    print("\n[G] VRP+VV2TX MA5 unbound asymmetric...")
    pos = run_ew_biv_unbound_asym(panel, "VP", "vv2tx_ma5", FWD, OOS_GAP, NW_LAGS)
    sim_biv_asym = _report(pos, daily_ret, "ub_asym_biv")
    plot_unbound_bivariate(
        "VRP", "VV2TX MA5", "20-day", "asym", C_BIV,
        sim_biv_asym, betas_biv, bah_sim, yh_biv, mu_20d,
        out_biv / "leveraged_asymmetric_VRP_+_VV2TX_MA5.png",
        c1=C_VRP, c2=C_VV2TX,
    )

    # ── H: VRP+VV2TX MA5 bivariate base_return_shift ─────────────────────────
    print("\n[H] VRP+VV2TX MA5 unbound base_return_shift...")
    pos = run_ew_biv_unbound_rolmu(panel, "VP", "vv2tx_ma5", FWD, OOS_GAP, NW_LAGS)
    sim = _report(pos, daily_ret, "ub_rolmu_biv")
    plot_unbound_bivariate(
        "VRP", "VV2TX MA5", "20-day", "rolmu", C_BIV,
        sim, betas_biv, bah_sim, yh_biv, mu_20d,
        out_biv / "leveraged_base_return_shift_VRP_+_VV2TX_MA5.png",
        c1=C_VRP, c2=C_VV2TX,
    )
    ...
    # ── Comparison: unbound-asymmetric VV2TX MA5 vs VRP+VV2TX MA5 vs Term ────
    print("\n[Comparison] Unbound asymmetric: VV2TX MA5 vs VRP+VV2TX MA5 vs VRP+Term Slope...")
    plot_unbound_comparison(
        sim_vv2tx_asym, sim_biv_asym, sim_term_asym, bah_sim,
        out_cmp / "leveraged_asymmetric_vv2tx_vs_vrp_vv2tx.png",
        sim_vrp=sim_vrp_asym,
    )

    print("\nDone.  Output files:")
    print(f"  {out_vv2tx.name}/leveraged_symmetric_VV2TX_MA5.png")
    print(f"  {out_vv2tx.name}/leveraged_asymmetric_VV2TX_MA5.png")
    print(f"  {out_vrp.name}/leveraged_symmetric_VRP.png")
    print(f"  {out_vrp.name}/leveraged_asymmetric_VRP.png")
    print(f"  {out_vrp.name}/leveraged_base_return_shift_VRP.png")
    print(f"  {out_biv.name}/leveraged_symmetric_VRP_+_VV2TX_MA5.png")
    print(f"  {out_biv.name}/leveraged_asymmetric_VRP_+_VV2TX_MA5.png")
    print(f"  {out_biv.name}/leveraged_base_return_shift_VRP_+_VV2TX_MA5.png")
    print(f"  {out_term.name}/leveraged_symmetric_VRP_+_Term_Slope.png")
    print(f"  {out_term.name}/leveraged_asymmetric_VRP_+_Term_Slope.png")
    print(f"  {out_term.name}/leveraged_base_return_shift_VRP_+_Term_Slope.png")
    print(f"  comparisons/leveraged_asymmetric_vv2tx_vs_vrp_vv2tx.png")
    print("=" * 72)
