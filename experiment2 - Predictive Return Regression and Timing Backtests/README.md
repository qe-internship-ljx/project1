...
|---|---|---|
| `VP` | Variance risk premium `IVar − CV` | `experiment1 - HAR VRP Computation/output/production_loop_rolling.parquet` (`.csv` fallback) via `load_vrp_series()` |
| `term_slope` | VIX futures term-structure slope β₀(t) | `Fassas_Hourvouliades_Replication.compute_vix_term_slope()` (daily cross-sectional OLS `price ~ TtM`) |
...
leveraged_strategies → plot`. External imports:
`Bekaert_Hoerova_Replication/har_model._nw_se` and
`Fassas_Hourvouliades_Replication.fh_replication.compute_vix_term_slope`.
...
Prerequisite: run `experiment1 - HAR VRP Computation/experiment.py` first (the
VRP loaders read its `output/`). Also needs sibling `Bekaert_Hoerova_Replication/`,
`Fassas_Hourvouliades_Replication/` and `data/`.
