"""One-time script: 7x6 extended Sharpe ratio table (all main models x 6 strategies)."""
import warnings
...
def fill_row(row_idx, pred1, pred2):
    ...
    pos = (run_ew_unbound_sym(panel, pred1, FWD, OOS_GAP, NW_LAGS) if pred2 is None
           else run_ew_biv_unbound_sym(panel, pred1, pred2, FWD, OOS_GAP, NW_LAGS))
    sr, ss = single_sharpe(pos)
    data[row_idx, 3] = sr
    windows[(row_idx, 3)] = ss.strftime("%Y-%m-%d")
    print(f"    lev_symmetric  SR={sr:+.3f}  stats_from={ss.date()}")

    pos = (run_ew_unbound_asym(panel, pred1, FWD, OOS_GAP, NW_LAGS) if pred2 is None
           else run_ew_biv_unbound_asym(panel, pred1, pred2, FWD, OOS_GAP, NW_LAGS))
    sr, ss = single_sharpe(pos)
    data[row_idx, 4] = sr
    windows[(row_idx, 4)] = ss.strftime("%Y-%m-%d")
    print(f"    lev_asymmetric SR={sr:+.3f}  stats_from={ss.date()}")

    pos = (run_ew_unbound_rolmu(panel, pred1, FWD, OOS_GAP, NW_LAGS) if pred2 is None
           else run_ew_biv_unbound_rolmu(panel, pred1, pred2, FWD, OOS_GAP, NW_LAGS))
    sr, ss = single_sharpe(pos)
    data[row_idx, 5] = sr
...
out_path = ROOT / "output" / "expanding_window" / "collage" / "sharpe_table_extended.png"
