...
Modules shared with Bekaert_Hoerova_Replication (called directly, not duplicated)
---------------------------------------------------------------------
...
# ── Shared modules from Bekaert_Hoerova_Replication ───────────────────────────
BH_DIR = Path(__file__).parent.parent / "Bekaert_Hoerova_Replication"
...
def load_implied_variance() -> pd.Series:
    ...
    """
    vix = load_vix()                              # from Bekaert_Hoerova_Replication.data_prep
    ivar = vix ** 2 / 12.0
    ivar.name = "IVar"
    return ivar
...
def main():
    ...
    # ── STEP 3: HAR in-sample & OOS (via Bekaert_Hoerova_Replication.har_model
    ...
    vix_ol = prod_rolling.loc[overlap_start:]
    vs_ol  = prod_vs
