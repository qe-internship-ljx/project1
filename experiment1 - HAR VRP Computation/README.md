# experiment1 — HAR VRP Computation

Production-grade daily **variance risk premium (VRP)** extraction, extending
the `Bekaert_Hoerova_Replication` Bekaert & Hoerova (2014) baseline with
day-by-day re-estimated HAR forecasts. This experiment produces the VRP time
series that `experiment2 - Predictive Return Regression and Timing Backtests`
(and `experiment3 - Nasdaq 100 Cross-Market`) consume.
...
1. **Panel build** — front-month ES returns → RV components (`RV1/RV5/RV22`,
   ...
   imported from `Bekaert_Hoerova_Replication` (`data_prep`, `har_model`), not duplicated.
...
| File | Role |
|---|---|
| `experiment.py` | The full pipeline (entry point `main()`). Config constants at the top: `PAPER_START/END/SPLIT`, `ROLL_WIN = 1000`, `EXP_TRAIN_START = 1990-01-02`, `EXP_OOS_START = 2006-01-01`, `PAPER_COEFS`. Reusable pieces: `production_loop`, `production_loop_expanding`, `plot_combined_vrp_summary` (also imported by `experiment3 - Euro Stoxx 50 Cross-Market`) |
...
Downstream: run this **before**
`experiment2 - Predictive Return Regression and Timing Backtests` and
`experiment3 - Nasdaq 100 Cross-Market`, which read
`output/production_loop_expanding.csv`
