...
from horizon_regression import (
    build_panel, compute_betas, compute_betas_bivariate,
...
def _rolling_mu(panel, fwd_col, oos_gap, rolling_window=500):
    sub = panel.dropna(subset=["VP", fwd_col]).copy()
    return (sub[fwd_col]
...
def main():
    ...
    print("  VVIX MA5 / VRP / VRP+VVIX MA5  x  sym / asym / rolmu")
    print("=" * 72)
    ...
    vvix_raw   = load_vvix()
    vvix_ma5_  = compute_vvix_ma5(vvix_raw)
    vix_spot   = load_vix_spot()
    term_slope = compute_vix_term_slope(load_vix_futures_term_structure())
    panel      = build_panel(vrp, es, vvix_raw, vvix_ma5_, vix_spot, term_slope)
    ...
    betas_vvix = compute_betas(panel, "vvix_ma5", "fwd_20d", oos_gap=OOS_GAP, nw_lags=NW_LAGS)
    betas_vrp  = compute_betas(panel, "VP",       "fwd_20d", oos_gap=OOS_GAP, nw_lags=NW_LAGS)
    betas_biv  = compute_betas_bivariate(panel, "VP", "vvix_ma5", "fwd_20d",
                                         oos_gap=OOS_GAP, nw_lags=NW_LAGS)
    print("    Done.")

    yh_vvix = _yhat_univariate(panel, "vvix_ma5", "fwd_20d", betas_vvix)
    yh_vrp  = _yhat_univariate(panel, "VP",       "fwd_20d", betas_vrp)
    yh_biv  = _yhat_bivariate(panel, "VP", "vvix_ma5", "fwd_20d", betas_biv)
    mu_20d  = _rolling_mu(panel, "fwd_20d", OOS_GAP, RW)

    out_vvix = OUTPUT / "expanding_window" / "VVIX MA5"
    out_vrp  = OUTPUT / "expanding_window" / "VRP"
    out_biv  = OUTPUT / "expanding_window" / "VRP + VVIX MA5"
    for d in (out_vvix, out_vrp, out_biv):
        d.mkdir(parents=True, exist_ok=True)

    # ── A: VVIX MA5 symmetric ────────────────────────────────────────────────
    print("\n[A] VVIX MA5 unbound symmetric...")
    pos = run_ew_unbound_sym(panel, "vvix_ma5", "fwd_20d", OOS_GAP, NW_LAGS)
    ...
    plot_unbound_univariate(
        ...
        out_vvix / "unbound_symmetric_VVIX_MA5.png",
        extra_title="\nFlat before VVIX activation (2006-03-06)",
    )

    # ── B: VVIX MA5 asymmetric ───────────────────────────────────────────────
    print("\n[B] VVIX MA5 unbound asymmetric...")
    pos = run_ew_unbound_asym(panel, "vvix_ma5", "fwd_20d", OOS_GAP, NW_LAGS)
    pos = pos.copy(); pos[pos.index < VVIX_ACT] = 0.0
    sim = _report(pos, daily_ret, "ub_asym_VVIX")
    plot_unbound_univariate(
        "VVIX MA5", "20-day", "asym", C_VVIX,
        sim, betas_vvix, bah_sim, yh_vvix, mu_20d,
        out_vvix / "unbound_asymmetric_VVIX_MA5.png",
        extra_title="\nFlat before VVIX activation (2006-03-06)",
    )

    # ── C: VRP symmetric ─────────────────────────────────────────────────────
    print("\n[C] VRP unbound symmetric...")
    pos = run_ew_unbound_sym(panel, "VP", "fwd_20d", OOS_GAP, NW_LAGS)
    sim = _report(pos, daily_ret, "ub_sym_VRP")
    plot_unbound_univariate(
        "VRP", "20-day", "sym", C_VRP,
        sim, betas_vrp, bah_sim, yh_vrp, mu_20d,
        out_vrp / "unbound_symmetric_VRP.png",
    )

    # ── D: VRP asymmetric ────────────────────────────────────────────────────
    print("\n[D] VRP unbound asymmetric...")
    pos = run_ew_unbound_asym(panel, "VP", "fwd_20d", OOS_GAP, NW_LAGS)
    sim = _report(pos, daily_ret, "ub_asym_VRP")
    plot_unbound_univariate(
        "VRP", "20-day", "asym", C_VRP,
        sim, betas_vrp, bah_sim, yh_vrp, mu_20d,
        out_vrp / "unbound_asymmetric_VRP.png",
    )

    # ── E: VRP base_return_shift ─────────────────────────────────────────────
    print("\n[E] VRP unbound base_return_shift...")
    pos = run_ew_unbound_rolmu(panel, "VP", "fwd_20d", OOS_GAP, NW_LAGS)
    sim = _report(pos, daily_ret, "ub_rolmu_VRP")
    plot_unbound_univariate(
        "VRP", "20-day", "rolmu", C_VRP,
        sim, betas_vrp, bah_sim, yh_vrp, mu_20d,
        out_vrp / "unbound_base_return_shift_VRP.png",
    )

    # ── F: VRP+VVIX MA5 bivariate symmetric ──────────────────────────────────
    print("\n[F] VRP+VVIX MA5 unbound symmetric...")
    pos = run_ew_biv_unbound_sym(panel, "VP", "vvix_ma5", "fwd_20d", OOS_GAP, NW_LAGS)
    ...
    plot_unbound_bivariate(
        ...
        out_biv / "unbound_symmetric_VRP_+_VVIX_MA5.png",
    )

    # ── G: VRP+VVIX MA5 bivariate asymmetric ─────────────────────────────────
    print("\n[G] VRP+VVIX MA5 unbound asymmetric...")
    pos = run_ew_biv_unbound_asym(panel, "VP", "vvix_ma5", "fwd_20d", OOS_GAP, NW_LAGS)
    sim = _report(pos, daily_ret, "ub_asym_biv")
    plot_unbound_bivariate(
        "VRP", "VVIX MA5", "20-day", "asym", C_BIV,
        sim, betas_biv, bah_sim, yh_biv, mu_20d,
        out_biv / "unbound_asymmetric_VRP_+_VVIX_MA5.png",
    )

    # ── H: VRP+VVIX MA5 bivariate base_return_shift ───────────────────────────
    print("\n[H] VRP+VVIX MA5 unbound base_return_shift...")
    pos = run_ew_biv_unbound_rolmu(panel, "VP", "vvix_ma5", "fwd_20d", OOS_GAP, NW_LAGS)
    sim = _report(pos, daily_ret, "ub_rolmu_biv")
    plot_unbound_bivariate(
        "VRP", "VVIX MA5", "20-day", "rolmu", C_BIV,
        sim, betas_biv, bah_sim, yh_biv, mu_20d,
        out_biv / "unbound_base_return_shift_VRP_+_VVIX_MA5.png",
    )

    print("\nDone.")
    print("  output/expanding_window/VVIX MA5/unbound_symmetric_VVIX_MA5.png")
    print("  output/expanding_window/VVIX MA5/unbound_asymmetric_VVIX_MA5.png")
    print("  output/expanding_window/VRP/unbound_symmetric_VRP.png")
    print("  output/expanding_window/VRP/unbound_asymmetric_VRP.png")
    print("  output/expanding_window/VRP/unbound_base_return_shift_VRP.png")
    print("  output/expanding_window/VRP + VVIX MA5/unbound_symmetric_VRP_+_VVIX_MA5.png")
    print("  output/expanding_window/VRP + VVIX MA5/unbound_asymmetric_VRP_+_VVIX_MA5.png")
    print("  output/expanding_window/VRP + VVIX MA5/unbound_base_return_shift_VRP_+_VVIX_MA5.png")
    print("=" * 72)
